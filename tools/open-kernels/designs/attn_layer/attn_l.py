...
def attn_l(w: In, xres: In, consts: In, kv: In, act: InOut, hdr: Out, *,
    ...
    for pb, ks in ((PB16, ks16), (PB32, ks32)):
        for i in range(pb // PER_CALL):
            ...
            ks.append(ExternalFunction(src.stem, source_file=str(src), arg_types=[elem_ty, x_ty, band_ty], include_dirs=inc))
    ...
    def gemv_body(win, xin, yout, *fns):
    ...
    for c in range(N_CORES):
        workers.append(Worker(gemv_body, fn_args=[of_w[c].cons(), of_x.cons(), of_y[c].prod(), *ks16, *ks32],
                              tile=Tile(c, 2), stack_size=0xD00))
