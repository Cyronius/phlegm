...
MoE (after the driver's `moeroute2`). `pos` (cached rows) is still a
CompileTime parameter (plan item 3).
...
meta | router W | sgw], kv (the KV cache), act (scratch; the new cache rows at AA_KVNEW).
Build (WSL): for p in 0 1: ATTN_POS=0 AX_PART=$p python build_design.py designs/layer_x/ax.py designs/layer_x/build_ax$p
"""
...
from layout import (AA_BYTES, AA_HP, AA_KVN, AA_KVNEW, AA_OG, AA_OUT, AA_QG, AA_RES, AA_ROUT, AA_XM, AA_XN,  # noqa: E402
                    CA_BYTES, CA_LNW, CA_META, CA_POSTLN, CA_RW, CA_SGW, KV_BYTES, KV_V_OFF, POOL_BYTES,
                    POOL_GATE, POOL_K, POOL_O, POOL_Q, POOL_V)
...
def ax(pool: In, xres: InOut, consts: In, kv: In, act: InOut, *, pos: CompileTime[int], part: CompileTime[int] = 0,
    ...
    def sequence(a_pool, c_xres, a_consts, a_kv, a_act, lni, lno, w_prods, x_prod, y_conss, ain_p, aout_c):
        if part == 0:
            tg_ln = TaskGroup()
            ...
            for tt in range(pos):
                pa_in.fill(ain_p, a_kv, bt(KV_ELEMS, tt * 512, 512))
                pa_in.fill(ain_p, a_kv, bt(KV_ELEMS, V_OFF + tt * 512, 512))
    ...
    rt = Runtime(sequence, [pool_ty, xres_ty, consts_ty, kv_ty, act_ty,
...
SPECIALIZE = {"pos": POS, "part": PART, "srchash": int(hashlib.sha1(_src).hexdigest()[:8], 16)}
