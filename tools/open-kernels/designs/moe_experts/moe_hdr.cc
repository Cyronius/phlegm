...
// copied out because release() frees the oldest element of the fifo.
//   x  <- xm
...
#pragma clang loop unroll(disable)
  for (unsigned j = 0; j < 2048; j += 32) {
    const v32b xv = aie::load_v<32>(xs + j);
    aie::store_v(x + j, xv);
    d = aie::mac(d, xv, aie::load_v<32>(sgw + j));
  }
