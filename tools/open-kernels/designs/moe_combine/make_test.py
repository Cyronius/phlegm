"""Test vectors for moe_combine: random y_e/xres/shared/xm, real shared_expert_gate
weight from the captured L0 pack (C:/caps/m0d/000118.bo @8192, bf16[2048]), router
weights from designs/router/ref_out.bin if present; fp64 reference."""
...
def main() -> int:
    ...
    for name, arr in (("rout", rout), ("y", y), ("xres", xres), ("shared", shared), ("xm", xm), ("sgw", sgw)):
    ...
    cfg = "\n".join([
        ...
        "run k rout y xres shared xm sgw out",
        "run k rout y xres shared xm sgw out",
        f"dump out {d}/y_out.bin {xres.nbytes}",
        "",
    ])
